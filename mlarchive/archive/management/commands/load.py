...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option('-b', '--break', dest='break',
            help='Break on error'),
        ...
        make_option('-n', '--dry-run', action='store_true', dest='dryrun', default=False,
            help='Perform a trial run with no messages saved'),
    ...
    def handle(self, *args, **options):
        ...
        if options.get('firstrun') and Legacy.objects.all().count() == 0:
            raise CommandError('ERROR: on firstrun, the legacy archive table is empty')
